...
| `src/commands/` | 每个子命令一个模块（`Args` + 入口函数）；`mod.rs` 提供共享 `Context`（bin-dir）与 `locate_tool`/`tool_not_found_error` | 纯逻辑 |
...
- 验证路径边界：`run C:\工具箱外\真实存在的文件`（绝对路径）与 `run ../xxx` 默认必须报 not found（退出码 127）；`run C:\工具箱内\绝对路径` 必须可运行；`add` 外部源后不带任何 flag 直接 `run` 必须可运行（Unix 符号链接路径）。